...
class Rbac {
    ...
    static function saveAccessList($authId=null) {
        if(null===$authId)   $authId = session(config('USER_AUTH_KEY'));
        // 如果使用普通权限模式，保存当前用户的访问权限列表
        // 对管理员开发所有权限
        if(config('USER_AUTH_TYPE') !=2 && !session(config('ADMIN_AUTH_KEY')) )
	...
	static public function checkLogin() {
        ...
        if(self::checkAccess()) {
            //检查认证识别号
            if(empty(session(config('USER_AUTH_KEY')))) {
                if(config('GUEST_AUTH_ON')) {
                    ...
                    // 禁止游客访问跳转到认证网关
                    header('location:' . config('USER_AUTH_GATEWAY'));
                }
            }
        }
        return true;
    ...
    static public function AccessDecision($appName='') {
        $appName = !empty($appName) ? $appName : request()->module();
        //检查是否需要认证
        if(self::checkAccess()) {
            //存在认证识别号，则进行进一步的访问决策
            ...
            if(empty(session(config('ADMIN_AUTH_KEY')))) {
                ...
                if(!isset($accessList[strtoupper($appName)][strtoupper(request()->controller())][strtoupper(request()->action())])) {
